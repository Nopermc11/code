...
local T1UIColor = {
    ...
    ["Logo Image"] = "rbxassetid://100472177647993",
...
local currcolor = {}
local Library = {};
Library.Options = {} -- FIX: Registry cho toggle/dropdown/slider theo id
...
ImageLabel.Image = "rbxassetid://100472177647993"
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,80,80") .. ")\">" .. tostring(TitleNameMain or "HDANH HUB") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
    ...
    -- FIX: Hỗ trợ cả SubTitle (Fluent UI style) lẫn Subtitle/Desc
    getgenv().MainDesc = Setting.Desc or Setting.SubTitle or Setting.Subtitle or ""
   ...
   TextLabelMain.Text = "<font color=\"rgb(0,0,0)\">" .. tostring(TitleNameMain or "HDANH HUB") .. "</font> <font color=\"rgb(255,255,255)\">" .. tostring(getgenv().MainDesc or "") .. "</font>"
	...
	local PageCounter = 1
		...
		-- FIX: Hỗ trợ cả table {Title=...} lẫn string thuần
		if type(PageName) == "table" then
			IconId = PageName.Icon or PageName.IconId or IconId
			PageName = PageName.Title or PageName.Name or PageName.Text or "Tab"
		end
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			local sectionFunction = {}
				...
				local function ButtonClick()
					...
					for _, cb in ipairs(_changedCallbacks) do pcall(cb, Default) end
				end
				
				ToggleButton.MouseButton1Click:Connect(ButtonClick)
				
				function toggleFunction.SetStage(value)
					local v = (value == true or value == 1)
					if v ~= Default then ButtonClick() end
				end
				-- FIX: compat aliases
				...
				function toggleFunction:OnChanged(cb)
					table.insert(_changedCallbacks, cb)
					return self
				end
				function toggleFunction.SetDesc(desc)
					pcall(function() ToggleDesc.Text = desc end)
				end
				
				-- FIX: Đăng ký vào Library.Options bằng id
				if idk and idk ~= "" then
					Library.Options = Library.Options or {}
					Library.Options[idk] = toggleFunction
				end
			...
			function sectionFunction:AddLabel(text)
                ...
                TextColor.RichText = true -- FIX: enable rich text for bold titles
				local labelFunction = {}
				...
				function labelFunction:SetTitle(text)
					TextColor.Text = text
				end
            ...
            function sectionFunction:AddDropdownSection(Setting)
               ...
               local dropdownSectionFunction = {}
                ...
                -- FIX: OnChanged compat
                local _dropdownChangedCallbacks = {}
                function dropdownFunction:OnChanged(cb)
                    table.insert(_dropdownChangedCallbacks, cb)
                    return self
                end
                -- Override SetValue để trigger callbacks
                local _origSetValue = dropdownFunction.SetValue
                function dropdownFunction:SetValue(value)
                    _origSetValue(self, value)
                    for _, cb in ipairs(_dropdownChangedCallbacks) do pcall(cb, value) end
                end
				local controlData = {
                    Name = Title,
                    Section = Section,
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    return keybindFunction
			...
			function sectionFunction:AddInput(idk, Setting)
				...
				local _inputChangedCallbacks = {}
				local textbox_function = {}
				if Default then Boxxx.Text = Default end
				function textbox_function.SetValue(Value)
					Boxxx.Text = Value
					if Callback then Callback(Value) end
					for _, cb in ipairs(_inputChangedCallbacks) do pcall(cb, Value) end
				end
				function textbox_function:OnChanged(cb)
					table.insert(_inputChangedCallbacks, cb)
					return self
				end
			...
			function sectionFunction:AddSlider(Setting)
				...
				RunService.RenderStepped:Connect(function()
					...
					if dragging and dragInput then
						local value = (Setting.Rouding or Setting.Rounding) and tonumber(string.format("%.".. (Setting.Rouding or Setting.Rounding or 1) .."f", (((tonumber(maxValue) - tonumber(minValue)) / SizeChia) * Bar.AbsoluteSize.X) + tonumber(minValue))) or math.floor((((tonumber(maxValue) - tonumber(minValue)) / SizeChia) * Bar.AbsoluteSize.X) + tonumber(minValue))
						pcall(function()
							callBackAndSetText(value)
						end)
						Bar.Size = UDim2.new(0, math.clamp(dragInput.Position.X - Bar.AbsolutePosition.X, 0, SizeChia), 0, 6)
				...
				local _sliderChangedCallbacks = {}
				local slider_function = {}
				function slider_function.SetValue(Value)
					GetSliderValue(Value)
				end
				function slider_function:OnChanged(cb)
					table.insert(_sliderChangedCallbacks, cb)
					return self
				end
				-- Patch callBackAndSetText để trigger OnChanged
				local _origCallBackAndSetText = callBackAndSetText
				callBackAndSetText = function(val)
					Sliderbox_2.Text = val
					if Callback then Callback(tonumber(val)) end
					for _, cb in ipairs(_sliderChangedCallbacks) do pcall(cb, tonumber(val)) end
				end
				local controlData = {
                    Name = TitleText,
                    Section = Section,
        ...
        -- FIX: Expose AddSection trực tiếp trên tab
        function pagefunc:AddSection(name, ...)
            return pageFunction:AddSection(name, ...)
        end
        
        -- FIX: Tạo default section ẩn để nhận các element gọi thẳng trên tab
        local _defaultSection = nil
        local function getDefaultSection()
            if not _defaultSection then
                _defaultSection = pageFunction:AddSection("_default_")
                -- Ẩn toàn bộ header của default section (Topsec + underline)
                pcall(function()
                    local sec = PageList:FindFirstChild("_default__Dot")
                    if sec then
                        local topsec = sec:FindFirstChild("Topsec")
                        if topsec then
                            topsec.Size = UDim2.new(1, 0, 0, 0)
                            topsec.Visible = false
                        end
                        -- Xóa border của default section
        ...
        -- FIX: Expose tất cả element methods trực tiếp trên tab
        function pagefunc:AddToggle(id, setting)
            return getDefaultSection():AddToggle(id, setting)
        end
        function pagefunc:AddButton(setting, callback)
            return getDefaultSection():AddButton(setting, callback)
        end
        function pagefunc:AddDropdown(id, setting)
            return getDefaultSection():AddDropdown(id, setting)
        end
        function pagefunc:AddSlider(id, setting)
            -- banana_cat gọi AddSlider("id", {Title=..., Min=..., Max=...})
            -- sectionFunction:AddSlider chỉ nhận 1 table
            local s = setting or id
            if type(s) ~= "table" then s = {Title = tostring(s)} end
            if type(id) == "string" and setting then s._id = id end
            return getDefaultSection():AddSlider(s)
        end
        function pagefunc:AddInput(id, setting)
            return getDefaultSection():AddInput(id, setting)
        end
        function pagefunc:AddLabel(text)
            return getDefaultSection():AddLabel(text)
        end
        function pagefunc:AddParagraph(setting)
            local title = (type(setting) == "table") and (setting.Title or setting.title or "") or tostring(setting or "")
            local content = (type(setting) == "table") and (setting.Content or setting.content or setting.Desc or "") or ""
            local text = (title ~= "" and content ~= "") and ("<b>" .. title .. "</b>\n" .. content)
                      or (title ~= "" and title)
                      or content
            local lf = getDefaultSection():AddLabel(text)
            -- Wrap để SetDesc cập nhật phần content, giữ title
            local _savedTitle = title
            if lf then
                local _origSetDesc = lf.SetDesc
                function lf:SetDesc(desc)
                    local newText = (_savedTitle ~= "" and desc ~= "") and ("<b>" .. _savedTitle .. "</b>\n" .. desc)
                                 or (_savedTitle ~= "" and _savedTitle)
                                 or desc
        ...
        function pagefunc:AddSeperator(text)
            return getDefaultSection():AddSeperator(text)
        end
        function pagefunc:AddKeyBind(setting, callback)
            return getDefaultSection():AddKeyBind(setting, callback)
        end
