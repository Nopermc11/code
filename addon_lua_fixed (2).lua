...
local redzlib = {
    ...
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(180, 80, 120)),
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(255, 190, 220)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(180, 80, 120))
}),
["Color Hub 2"] = Color3.fromRGB(60, 30, 48),
["Color Stroke"] = Color3.fromRGB(255, 175, 210),
["Color Theme"] = Color3.fromRGB(255, 200, 225),
["Color Text"] = Color3.fromRGB(255, 242, 250),
["Color Dark Text"] = Color3.fromRGB(240, 170, 200)
        }
    },
	Info = {
		Version = "1.1.0"
...
local OldGui = CoreGui:FindFirstChild("redz Library V5")
if OldGui then OldGui:Destroy() end
if gethui then
	local OldGui2 = gethui():FindFirstChild("redz Library V5")
...
if not success or not ScreenGui then
	-- Fallback: dùng gethui() nếu CoreGui bị block (thường gặp trên Xeno PC)
	local GuiParent
	if gethui then
		GuiParent = gethui()
	else
	...
	ScreenGui = Create("ScreenGui", GuiParent, {
		Name = "redz Library V5",
		ResetOnSpawn = false,
		IgnoreGuiInset = true,
	}, {
...
local function CreateTween(Configs)
	...
	local TweenInfoObj = TweenInfo.new(Time, Enum.EasingStyle.Quint)
	
	local Tween = TweenService:Create(Instance, TweenInfoObj, {[Prop] = NewVal})
	Tween:Play()
	if TweenWait then
...
local function MakeDrag(Instance)
	task.spawn(function()
		...
		local function Update(Input)
			...
			CreateTween({Instance, "Position", Position, 0.05})
		end
...
function redzlib:SetScale(NewScale)
	...
	if ScaleObj then ScaleObj.Scale = NewScale end
end
...
function redzlib:MakeWindow(Configs)
	...
	function Window:MinimizeBtn()
		...
		if Minimized then
			MinimizeButton.Image = "rbxassetid://100472177647993"
	...
	function Window:MakeTab(paste, Configs)
		...
		function Tab:AddSlider(Configs)
			...
			local function ControlPos()
				local APos = PlayerMouse.X - BaseMousePos.AbsolutePosition.X
